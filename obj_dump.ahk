...
;I just add a 9-byte header to identify the saved AutoIMG objects (BlassGO)

ObjDump(obj,ByRef var:="",mode:=0,header:="_"){
  If IsObject(var){ ; FileAppend mode
    if !FileDelete(obj)
       return
    f:=FileOpen(obj,"rw-rwd","CP0"),f.Write("AutoIMG_" . header),VarSetCapacity(v,sz:=RawObjectSize(var,mode)+8,0)
...
ObjHeader(file){
    if !FileExist(file)
       return
    f:=FileOpen(file, "r")
    return (InStr(header:=f.Read(9),"AutoIMG_")?header:""),f.Close()
}
